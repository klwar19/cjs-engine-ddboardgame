...
window.CJS.CampaignUI = (() => {
  ...
  return Object.freeze({
    ...
    // The manual quest builder (`_openQuestModal`, 475 lines) still lives
    // in JS — it shares `_randomizedQuestTemplate` / `_inferObjectiveKind`
    // / `_questBuilderMiniGame` with quest data flows. The add-quest TS
    // handler (manual-builders.ts) wraps it as a thin dispatcher so the
    // action contract is registry-backed even while the modal stays JS;
    // H.4 ports the body + its data builders together. (The manual event
    // builder, GM override, and scene builder bridges all dropped in H.4 —
    // those modals call their TS ports directly.)
    openQuestModal: (prefill) => _openQuestModal(prefill || {}),
    // `openGmOverride` + `openManualSceneBuilder` bridges removed in Phase
    // H.4 — the GM override modal ported to `action-handlers/gm-override.ts`
    // and the scene builder to `action-handlers/scene-builder.ts`; the
    // gm-override / gm-member-override / story-manual-note handlers call
    // those TS modules directly.
