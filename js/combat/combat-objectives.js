...
// Pluggable battle objective types — replaces the implicit "kill all enemies"
// win condition. Each objective declares: setup (initial state), tick (after
// each turn-end), evaluate (returns { winner, reason } or null), and
// describe (UI label/progress).
//
// Supported kinds:
//   kill_all       — default. Defeat all enemy units. (Back-compat.)
//   escort         — Keep a designated VIP unit alive for N rounds OR until
//                    it reaches a target cell. VIP death = loss.
//   capture_point  — Stand on (or adjacent to) a target zone for Y *consecutive*
//                    rounds with at least one player unit and no enemy. Enemy
//                    contesting resets the counter.
//   survival       — Endure until round Y. Reinforcements (script-only) can be
//                    used as flavor; the win condition is reaching the round
//                    threshold without total wipe.
//   assassination  — Kill the designated target unit, then have any surviving
//                    player unit step onto an escape tile (or simply survive
//                    one more round if no escape tile is set).
//
...
window.CJS.CombatObjectives = (() => {
  ...
  // Build the live objective tracker. Returns null if no special objective
  // is configured (callers fall back to the legacy kill_all check).
  /**
  ...
  // Called by combat-manager after each turn-end (BEFORE _checkBattleEnd's
  // default logic). Returns { winner, reason } if the battle should end now,
  // or null to continue.
  /**
  ...
  function _evalAssassination(tracker, state, ctx) {
    ...
    if (tracker.escapeCells.length) {
      const survivors = Object.values(state.units || {})
  ...
  function _findUnit(state, id) {
    ...
    return state.units[id] || Object.values(state.units).find((u) => u.baseId === id || u.instanceId === id) || null;
  }
   ...
   * @param {object} tracker
   * @param {object} combatState
   * @returns {{ title: string, detail: string, progressPct: number, icon: string, kind: string }}
   */
  ...
  // Optional: returns cells the renderer should highlight (capture zones,
  // escape tiles, escort goal). Used by combat-ui to overlay markers.
  ...
  // Fire reinforcements when survival mode hits the configured round.
  // Called by combat-manager after _advanceInitiative resets round; we use
  // CombatManager.gmAddUnit to add the reinforcements so they fold into the
  // current battle cleanly.
