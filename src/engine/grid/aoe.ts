// aoe.ts — Tier 3 TS port of js/grid/aoe.js (engine cluster: grid). AoE shape
// calculation: circle/disk/line/cone/cross/x/row/column. Returns sets of [r, c]
// cells for a given shape + origin + parameters. Pure functions — accept grid
// dims as arguments.
// Used by: grid-engine, action-handler, effect-resolver.
//
// Exports `AoE` and installs window.CJS.AoE (rides the CJSNamespace index
// signature). Bodies verbatim.
...
// For cardinal directions: a triangular wedge. For diagonal: quarter-disk.
...
// ── PUBLIC API ─────────────────────────────────────────────────────
export const AoE = Object.freeze({
  DIRECTIONS, directionFrom,
  circle, disk, line, cone, cross, xShape, row, column,
  getCellsForShape, unitsInCells
});

// Runtime compatibility install — keep window.CJS.AoE identical to the legacy
