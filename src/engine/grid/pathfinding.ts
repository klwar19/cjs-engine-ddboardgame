// pathfinding.ts — Tier 3 TS port of js/grid/pathfinding.js (engine cluster:
// grid). Movement range + shortest-path search on the combat grid. Handles
// multi-cell units (footprint must fit at every step) and per-terrain movement
// costs. Pure functions — accept grid state as arguments, no module state.
// Reads: window.CJS.CONST (TERRAIN_TYPES, UNIT_SIZES).
// Used by: grid-engine, ai-targeting/ai-controller.
//
// Exports `Pathfinding` and installs window.CJS.Pathfinding (rides the
...
// We take the MAXIMUM cost across all footprint cells — the slowest cell you're
// now standing on governs how tiring this step was.
...
// Given current pos and destination, return best anchor cell to move to using
// maxMP this turn. Destination doesn't need to be reachable this turn — we just
// walk as far toward it as we can.
...
// ── PUBLIC API ─────────────────────────────────────────────────────
export const Pathfinding = Object.freeze({
  reachableCells,
  findPath,
  stepToward,
  nearestEmptyCell
