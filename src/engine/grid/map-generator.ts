// map-generator.ts — Tier 3 TS port of js/grid/map-generator.js (engine cluster:
// grid). Procedural battle-map generation for quick battles: per-theme obstacle/
// terrain weights, borders, spawn-zone computation, connectivity repair, and
// footprint-aware unit placement. Reads window.CJS.CONST. Uses Math.random.
// Used by: battle-setup.
//
// Exports `MapGenerator` and installs window.CJS.MapGenerator (rides the
...
function _weightedPick(weights) {
  const entries = Object.entries<any>(weights);
...
// Runtime compatibility install — keep window.CJS.MapGenerator identical to the
// legacy IIFE so every existing consumer (and the vanilla engine) is unchanged.
window.CJS = window.CJS || ({} as CJSNamespace);
window.CJS.MapGenerator = MapGenerator;
