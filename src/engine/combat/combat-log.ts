// combat-log.ts — Tier 3 TS port of js/combat/combat-log.js (engine cluster:
// combat). Records every combat event with full context and narrator-ready
// tags. The log is the single source of truth for "what happened this turn" —
...
// combat-ui, narrator-engine.
//
// Exports the typed `CombatLog: CJSCombatLog` AND installs window.CJS.CombatLog
// as a side effect. Module-level `let` state replaces the IIFE closure; bodies
// verbatim.

// ── STATE ──────────────────────────────────────────────────────────
let _entries: CJSLogEntry[] = [];
let _nextId = 1;
let _subscribers: Array<(entry: CJSLogEntry) => void> = [];  // called on each new entry
let _currentTurn = 1;
// 'setup' | 'turn_start' | 'action' | 'resolution' | 'turn_end'
let _currentPhase = 'setup';

// ── CORE: RECORD ────────────────────────────────────────────────────
function record(entry): CJSLogEntry {
  const full: CJSLogEntry = {
    id:        _nextId++,
    turn:      _currentTurn,
    phase:     _currentPhase,
...
// ── PUBLIC API ─────────────────────────────────────────────────────
export const CombatLog: CJSCombatLog = Object.freeze({
  // Generic
  record,
  // Shaped recorders
