// enemy-modifiers.ts — Tier 3 TS port of js/combat/enemy-modifiers.js (engine
// cluster: combat). Procedural prefix modifiers for normal monsters (small
// twists, not elite templates). Bosses/uniques/authored setpieces opt out unless
// a placement pins a modifier. No external deps (uses Math.random).
//
// Exports `EnemyModifiers` and installs window.CJS.EnemyModifiers (rides the
...
function pickModifier(baseUnit, ctx: any = {}) {
...
function rollAndApply(baseUnit, ctx: any = {}) {
...
function _contextChance(explicitChance, ctx: any = {}) {
...
export const EnemyModifiers = Object.freeze({
  shouldModify,
  pickModifier,
  applyModifier,
  rollAndApply,
...
// Runtime compatibility install — keep window.CJS.EnemyModifiers identical to the
// legacy IIFE so every existing consumer (and the vanilla engine) is unchanged.
window.CJS = window.CJS || ({} as CJSNamespace);
window.CJS.EnemyModifiers = EnemyModifiers;
