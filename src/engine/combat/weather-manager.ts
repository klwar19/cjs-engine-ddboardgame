// weather-manager.ts — Tier 3 TS port of js/combat/weather-manager.js (engine
// cluster: combat). Global battlefield environment ("weather") living on
// combat-manager _state.environment. Applies elemental damage multipliers, stat
// modifiers, periodic ticks, and status-duration adjustments.
//
// Reads: window.CJS DataStore/CombatLog/DamageCalc.
// Used by: combat-manager (tick), damage-calc (mods), stat-compiler (stat mods),
// status-manager (duration mods), effect-resolver (environment_set/clear).
//
// Exports `Weather` and installs window.CJS.Weather as a side effect (rides the
// CJSNamespace index signature). The legacy module returned a NON-frozen object,
// so the export is intentionally left unfrozen to preserve that. Bodies verbatim.
...
function _applyPeriodicTick(state, def) {
  ...
  if (amount === 0) return;
  const units: any[] = Object.values(state.units || {});
...
// NOTE: the legacy module returned a plain (unfrozen) object; preserved here.
