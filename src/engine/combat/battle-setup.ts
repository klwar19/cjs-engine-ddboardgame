// battle-setup.ts — Tier 3 TS port of js/combat/battle-setup.js (engine cluster:
// combat). Pre-combat setup screen for quick random battles and premade map
// launches: party/monster selection, theme/size map generation, and the
// premade-encounter party-placement editor. DOM-driven (renders into a container).
//
// Reads: window.CJS DataStore/MapGenerator/CONST/ContentManager.
//
// Exports `BattleSetup` and installs window.CJS.BattleSetup (rides the
// CJSNamespace index signature). Bodies verbatim. `_container` is dynamic DOM
// (any), so the render/query/bind code is untyped exactly as before.

const DS = () => window.CJS.DataStore;
const MG = () => (window.CJS as any).MapGenerator;
const C = () => window.CJS.CONST;
const CM = () => (window.CJS as any).ContentManager;

let _container: any = null;
let _onStart: any = null;

let _mode = 'quick';
let _selectedParty: any[] = [];
let _selectedMonsters: any[] = [];
let _selectedTheme = 'random';
let _mapSize = 8;
let _generatedMap: any = null;
let _selectedEncounterId: any = null;
let _selectPlacements: Record<string, any> = {};
let _activePlacementId: any = null;
let _placementKey = '';
...
export const BattleSetup = Object.freeze({
  init,
  show,
  hide,
  reset
