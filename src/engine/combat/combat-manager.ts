// combat-manager.ts — Tier 3 TS port of js/combat/combat-manager.js (engine
// cluster: combat). The turn-loop state machine: owns the live combat state,
// drives phases (turn_start → action → resolution → turn_end), routes control to
// AI or manual input, runs procedural enemy modifiers, weather ticks, objectives,
// and exposes GM controls.
//
// Reads: window.CJS DataStore/StatCompiler/GridEngine/ActionHandler/AIController/
// StatusManager/EffectResolver/CombatSettings/CombatLog/Formulas/CONST/
// DiceService/AudioManager/AnimationBus/Weather/StateTools/CombatObjectives/
// EnemyModifiers.
//
// Exports `CombatManager`; installs window.CJS.CombatManager via one documented
// cast. The runtime object is intentionally NOT frozen (tests stub specific
// functions such as getEnvironment), exactly like the legacy module. Bodies
// verbatim. `_state` and the unported/dynamic singletons are `any`.

const DS  = () => window.CJS.DataStore;
const SC  = (): any => window.CJS.StatCompiler;
const GE  = (): any => window.CJS.GridEngine;
const AH  = (): any => window.CJS.ActionHandler;
const AI  = (): any => window.CJS.AIController;
...
const D   = (): any => window.CJS.DiceService || window.CJS.Dice;
const AM  = (): any => window.CJS.AudioManager;
const AB  = (): any => window.CJS.AnimationBus;
const WX  = (): any => window.CJS.Weather;
const ST  = (): any => window.CJS.StateTools;
const OBJ = (): any => window.CJS.CombatObjectives;
const EM  = (): any => window.CJS.EnemyModifiers;
...
function _procModifierBudget(enc: any = {}, enemyCount = 0) {
...
function _procModifierChance(enc: any = {}, placement: any = {}) {
...
function _modifierTags(enc: any = {}, base: any = null) {
  const tags = new Set();
  const add = (values) => {
...
function startEncounter(encounterId) {
  ...
  const unitObjects: Record<string, any> = {};
  const initiative = [];
...
function _beginCurrentUnitTurn() {
  ...
  ER().fireTrigger('on_turn_start', {
    unit, allUnits: Object.values<any>(_state.units),
      ...
      if (_td && _td.effect) {
        ...
        if (_tEffect) {
          ER().executeEffect(_tEffect, {
            ...
            allUnits: Object.values<any>(_state.units),
            turnNumber: _state.roundNumber
          });
          Log().record({
...
function _afterActionHook(unit, action, result) {
  ...
  if (SM()) {
    const allUnits = Object.values<any>(_state.units);
  ...
  for (const u of Object.values<any>(_state.units)) {
...
function _endCurrentUnitTurn() {
  ...
  if (lastInRound && WX()) {
    ...
    if (prevId !== _state.environment?.id) {
      ...
      for (const u of Object.values<any>(_state.units)) WX().applyStatModsToUnit(u, _state);
    }
  }
...
function _checkBattleEnd() {
  ...
  const all = Object.values<any>(_state.units);
  ...
  for (const u of Object.values<any>(_state.units)) {
    if ((u.currentHP || 0) > 0) {
      ER().fireTrigger('on_battle_end', {
        unit: u, allUnits: Object.values<any>(_state.units),
        turnNumber: _state.roundNumber
      });
    }
  }
...
function getUnits() {
  return _state ? Object.values<any>(_state.units) : [];
}
...
function gmAddUnit(baseId, r, c, opts: any = {}) {
  if (!_state) return { success: false, reason: 'no_combat' };
  ...
  try { ER().fireTrigger('on_battle_start', { unit: compiled, allUnits: Object.values<any>(_state.units), turnNumber: _state.roundNumber }); } catch (e) {}
  _notify();
...
function gmApplyStatus(unit, statusId, duration) {
  ...
  SM().processRecompileRequests(Object.values<any>(_state.units), (baseId) =>
    DS().get('characters', baseId) || DS().get('monsters', baseId));
  _notify();
...
// Runtime compatibility install — keep window.CJS.CombatManager identical to the
// legacy IIFE (intentionally NOT frozen so tests can stub functions). The cast
// bridges the historical CJSCombatManager interface, whose declared surface is a
// subset of the runtime object (e.g. the gm* controls).
window.CJS = window.CJS || ({} as CJSNamespace);
window.CJS.CombatManager = CombatManager as unknown as CJSCombatManager;
