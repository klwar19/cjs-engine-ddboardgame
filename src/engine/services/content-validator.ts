// content-validator.ts — Tier 3 TS port of js/services/content-validator.js
// (engine cluster: services). Cross-references DataStore content and reports
// broken pointers, missing images, and obvious shape mismatches. Designed to run
// in-browser via the debug console and in Node test harnesses that pre-populate
// DataStore.
//
// Report shape:
//   { ranAt, errors: [{category,id,message,hint?}], warnings: [...],
//     stats: { totalChecked, byCategory } }
//
// Reads: window.CJS.DataStore (does not mutate state).
// Used by: dev-console, programmatic devs.
//
// Exports the `ContentValidator` API object AND installs
// window.CJS.ContentValidator as a side effect (rides the CJSNamespace index
// signature — there is no dedicated interface). Bodies verbatim.
...
const RULES: Record<string, Array<(entry: any, ctx?: any) => any>> = {
  skills: [
...
function run() {
  const errors = [];
  const warnings = [];
  const stats = { totalChecked: 0, byCategory: {} as Record<string, number> };
  if (!DS()) {
...
export const ContentValidator = Object.freeze({ run, summary, addRule, RULES });

// Runtime compatibility install — keep window.CJS.ContentValidator identical to
// the legacy IIFE so every existing consumer (and the vanilla engine) is unchanged.
window.CJS = window.CJS || ({} as CJSNamespace);
(window.CJS as any).ContentValidator = ContentValidator;
