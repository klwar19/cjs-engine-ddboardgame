// effect-resolver.ts — Tier 3 TS port of js/effects/effect-resolver.js (engine
// cluster: effects). THE SPINE. Given a trigger event + combat context, gathers
// every relevant effect from every relevant unit, evaluates conditions, and
// executes each one's action. Called at every combat phase by combat-manager,
// action-handler, damage-calc, status-manager.
//
// Reads: window.CJS DataStore/ValueCalc/Conditions/DamageCalc/StatusManager/
// GridEngine/AoE/CombatLog/CONST/Weather/CombatManager/DiceService.
//
// Exports `EffectResolver` and installs window.CJS.EffectResolver as a side
// effect (rides the CJSNamespace index signature). Bodies verbatim. The engine
// singletons are reached through `any`-typed accessors because several
// (DamageCalc/StatusManager/CombatLog/…) are still vanilla JS with partial or no
// ambient interface; their dynamic shape is preserved exactly.
...
const AoE  = () => (window.CJS as any).AoE;
const Log  = (): any => window.CJS.CombatLog;
const C    = () => window.CJS.CONST;

// ── MAIN ENTRY: FIRE TRIGGER ──────────────────────────────────────
// trigger: e.g. 'on_hit', 'on_kill', 'on_turn_start'
// context: { unit, target, attacker, damageDealt, damageReceived, overkill,
//   element, damageType, isCritical, skillUsed, turnNumber, allUnits, storedResults }
...
function _actHeal(effect, caster, targets, value, ctx?) {
  const healed = [];
  for (const t of targets) {
...
function _actUltimateReroll(effect, caster, ctx) {
  const DS: any = (window.CJS as any).DiceService;
  if (!DS?.rerollLast) {
...
// ── PUBLIC API ─────────────────────────────────────────────────────
export const EffectResolver = Object.freeze({
  fireTrigger,
  executeEffect,
  executeSingleEffect
});

// Runtime compatibility install — keep window.CJS.EffectResolver identical to the
// legacy IIFE so every existing consumer (and the vanilla engine) is unchanged.
window.CJS = window.CJS || ({} as CJSNamespace);
window.CJS.EffectResolver = EffectResolver;
