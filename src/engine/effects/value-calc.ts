// value-calc.ts — Tier 3 TS port of js/effects/value-calc.js (engine cluster:
// effects). Resolves effect value expressions: given a value + source + combat
// context, returns the final number. Pure function — no state.
// Reads: window.CJS.Dice.
// Used by: effect-resolver, damage-calc, stat-compiler (via window.CJS.ValueCalc).
//
// Exports `ValueCalc` and installs window.CJS.ValueCalc as a side effect
// (rides the CJSNamespace index signature). Bodies verbatim.

const Dice = () => window.CJS.Dice;

// ── MAIN RESOLVER ──────────────────────────────────────────────────
// context = {
//   caster, target, damageDealt, damageReceived, overkill, storedResults,
//   stackCount, turnNumber, unitsAliveAlly, unitsAliveEnemy
// }
...
// ── PUBLIC API ─────────────────────────────────────────────────────
export const ValueCalc = Object.freeze({
  resolve,
  describeValue,
  estimate
});

// Runtime compatibility install — keep window.CJS.ValueCalc identical to the
// legacy IIFE so every existing consumer (and the vanilla engine) is unchanged.
window.CJS = window.CJS || ({} as CJSNamespace);
window.CJS.ValueCalc = ValueCalc;
