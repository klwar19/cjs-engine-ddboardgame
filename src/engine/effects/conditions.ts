// conditions.ts — Tier 3 TS port of js/effects/conditions.js (engine cluster:
// effects). Evaluates condition strings for effects and AI rules. Parses
// compound logic like "hp_below_30 AND has_status_burn OR is_type_beast".
// Pure functions — no state (receives combat context as argument).
// Used by: effect-resolver, ai-conditions (via window.CJS.Conditions).
//
// Exports `Conditions` and installs window.CJS.Conditions as a side effect
// (rides the CJSNamespace index signature). Bodies verbatim.

// ── MAIN EVALUATOR ─────────────────────────────────────────────────
// conditions: string or array of strings
// context: { unit, target, attacker, damageType, element, isCritical,
//   skillUsed, gridEngine, turnNumber, combatState }
