// effect-registry.ts — Tier 3 TS port of js/effects/effect-registry.js (engine
// cluster: effects). Master effect library: CRUD (delegating to DataStore),
// templates, override merging, auto-description, validation.
// Reads: window.CJS.DataStore, window.CJS.ValueCalc, window.CJS.Conditions,
// window.CJS.CONST.
// Used by: all editors, effect-resolver, stat-compiler (via window.CJS.EffectRegistry).
//
// Exports `EffectRegistry` and installs window.CJS.EffectRegistry as a side
// effect (rides the CJSNamespace index signature). Bodies verbatim.
...
// ── PUBLIC API ─────────────────────────────────────────────────────
export const EffectRegistry = Object.freeze({
  // Templates
  createBlankEffect,
  // CRUD
...
// Runtime compatibility install — keep window.CJS.EffectRegistry identical to the
// legacy IIFE so every existing consumer (and the vanilla engine) is unchanged.
window.CJS = window.CJS || ({} as CJSNamespace);
window.CJS.EffectRegistry = EffectRegistry;
