// content-manager.ts — Tier 3 TS port of js/core/content-manager.js (engine
// cluster: core). Manifest-aware data orchestration for multi-file content
// loading, editing, validation, migration, and save/export assembly.
//
// Reads: window.CJS.DataStore, window.CJS.UI (editor modal), window.CJS.CONST.
// Used by: data-store (buildNewRecord/getManifest/getLoadMode), every
// entry-*.js bootstrap (loadDefaultData), the editor, and the dev data watcher.
//
// Exports the `ContentManager` API object AND installs window.CJS.ContentManager
// as a side effect. There is no CJSContentManager interface (it rides the
// CJSNamespace index signature), so the export is the accurately-inferred shape;
// bodies are verbatim apart from TS annotations on the module-level mutable
// state, the dynamic option bags, and the editor DOM lookups.

const DS = () => window.CJS.DataStore;
const UI = () => (window.CJS as any).UI;
...
const CATEGORY_TO_TYPE = Object.freeze(
  Object.entries(TYPE_TO_CATEGORY).reduce((out: Record<string, string>, [type, category]) => {
    out[category] = type;
    return out;
...
let _fileEntries: any[] = [];
let _fileHeaders: Record<string, any> = {};
let _dirtyFiles: Set<any> = new Set();
let _loadMode = 'legacy';
let _filters = { scope: 'all', world: 'all' };
let _validationIssues: any[] = [];
let _validationIndex: Record<string, number> = {};
let _lastMigration: any = null;
...
function _entriesFromDocument(doc, entry) {
  if (entry.category === 'effects') {
    ...
    return Object.entries(effects)
      ...
      .map(([id, effect]) => ({ ...(effect as any), id: (effect as any).id || id }));
  }
...
function _loadManifestDocuments(manifest, docs, options: any = {}) {
  _bindStore();
  _manifest = manifest;
...
function createEntry(type, defaults, onCreated?) {
  ...
  const scopeSel = body.querySelector('#cm-create-scope') as HTMLSelectElement;
  const worldGroup = body.querySelector('#cm-create-world-group') as HTMLElement;
  const worldSel = body.querySelector('#cm-create-world') as HTMLSelectElement | null;
  const pathBox = body.querySelector('#cm-create-path') as HTMLElement;
...
function buildFileMap(options: any = {}) {
  if (!_manifest) return {};
...
async function migrateLegacyData() {
  ...
  const materialMap = {};
  for (const monster of Object.values(legacyBundle.monsters || {}) as any[]) {
    ...
    doc: {
      ...
      effects: Object.fromEntries(
        Object.entries(masterEffects.effects || {})
          .filter(([id]) => !String(id).startsWith('__COMMENT'))
          .map(([id, effect]) => [id, { ..._stripMeta(effect), id: (effect as any).id || id }])
      )
    }
  });
    ...
    doc: {
      ...
      entries: _sortById(
        Object.values(legacyBundle.characters || {})
          .filter((character: any) => WORLD_UNIVERSAL_IDS.characters.has(character.id))
          .map((character: any) => ({
    ...
    doc: {
      ...
      entries: _sortById(
        Object.values(legacyBundle.passives || {})
          .filter((passive: any) => WORLD_UNIVERSAL_IDS.passives.has(passive.id))
          .map((passive: any) => ({
            ..._stripMeta(passive),
    ...
    doc: {
      ...
      entries: _sortById(
        Object.values(legacyBundle.skills || {})
          .filter((skill: any) => WORLD_UNIVERSAL_IDS.skills.has(skill.id))
          .map((skill: any) => ({
            ..._stripMeta(skill),
    ...
    if (entry.category === 'items') {
      docs.push({
        ...
        doc: {
          _file: _clone(_fileHeaders[entry.path] || _defaultHeader(entry)),
          entries: _sortById(
            Object.values(legacyBundle.items || {}).map((item: any) => ({
              ..._stripMeta(item),
    ...
    if (entry.category === 'monsters') {
      docs.push({
        ...
        doc: {
          _file: _clone(_fileHeaders[entry.path] || _defaultHeader(entry)),
          entries: _sortById(
            Object.values(legacyBundle.monsters || {}).map((monster: any) => ({
              ..._stripMeta(monster),
    ...
    if (entry.category === 'encounters') {
      docs.push({
        ...
        doc: {
          _file: _clone(_fileHeaders[entry.path] || _defaultHeader(entry)),
          entries: _sortById(
            Object.values(legacyBundle.encounters || {}).map((encounter: any) => ({
...
export const ContentManager = Object.freeze({
  buildFileMap,
  buildNewRecord,
  createEntry,
  reloadFile,
...
// Runtime compatibility install — keep window.CJS.ContentManager identical to the
// legacy IIFE so every existing consumer (and the vanilla engine) is unchanged.
window.CJS = window.CJS || ({} as CJSNamespace);
(window.CJS as any).ContentManager = ContentManager;
