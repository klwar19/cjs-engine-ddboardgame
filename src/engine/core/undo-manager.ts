// undo-manager.ts — Tier 3 TS port of js/core/undo-manager.js (engine cluster:
// core). Centralized undo/redo stack for all editor operations. Integrated into
// DataStore — every create/update/replace/remove automatically pushes to the
// stack (when enabled).
//
// Reads: window.CJS.DataStore (applies reverse/forward ops directly).
// Used by: data-store.js (push), editor (undo/redo buttons + Ctrl+Z).
//
// Exports the typed `UndoManager: CJSUndoManager` AND installs
// window.CJS.UndoManager as a side effect. Module-level `let` state replaces the
// IIFE closure — behavior is identical.

const MAX_HISTORY = 50;

let _stack: CJSUndoEntry[] = [];
// Pointer to last applied action; -1 means stack is empty.
let _pointer = -1;
// When false (bulk import/seed), push() is a no-op.
let _enabled = true;
let _subscribers: Array<(state: CJSUndoState) => void> = [];
...
function _applyReverse(entry) {
  var DS: any = window.CJS.DataStore;
  switch (entry.action) {
    case 'create':
...
// ── PUBLIC API ────────────────────────────────────────────────
export const UndoManager: CJSUndoManager = Object.freeze({
  push: push,
  undo: undo,
  redo: redo,
  canUndo: canUndo,
