// ai-controller.ts — Tier 3 TS port of js/ai/ai-controller.js (engine cluster:
// ai). Evaluates a unit's aiRules[] in priority order and returns a decision
// ({ type: skill/attack/move/defend/end_turn, … }); falls back to archetype
// defaults (aggressive/support/sniper/tank/boss/swarmer/coward/…) with emergency-
// heal preemption, weapon/cooldown/MP/AP/ultimate gating, and pathfinding-aware
// move-toward/away/flee.
//
// Reads: window.CJS AIConditions/AITargeting/GridEngine/DataStore/Pathfinding/
// SkillResolver/ActionHandler/CONST/CombatLog.
// Used by: combat-manager.
//
// Exports `AIController`; installs window.CJS.AIController via one documented cast
// (the export keeps test-only _buildContext/_decodeRule/_archetypeDefault beyond
// the historical interface). Bodies verbatim.

const AIC = (): any => window.CJS.AIConditions;
const AIT = (): any => window.CJS.AITargeting;
const GE  = (): any => window.CJS.GridEngine;
const DS  = () => window.CJS.DataStore;
const PF  = (): any => window.CJS.Pathfinding;
...
function _tryUseSkill(unit, skillId, targetSpec, ctx) {
  ...
  // unit doesn't meet it. Skip the skill entirely so the AI can fall through
  // to a viable option.
...
function _tryMoveToward(unit, targetSpec, ctx) {
  ...
  // Find an attacker anchor cell that puts the attacker's footprint ADJACENT
  // to the target's footprint. Accounts for the ATTACKER's size too.
  const tgt = pick.unit;
  const candidates = [];
  ...
  // Final fallback: pick whichever reachable cell minimises footprint distance.
...
function _archetypeDefault(unit, ctx) {
  // Characters typically have no authored behaviorAI; treat a unit with a
  // mostly-support skill kit as 'support'. Monsters/explicit archetypes keep it.
  ...
  // If any ally (or self) is critically wounded and we have a ready heal-type
  // skill, use it BEFORE falling through to basic attack.
  ...
  if (!unit.turnState?.hasMoved) {
    ...
    // Support units close the gap to allies (not enemies) when they have no
    // offensive option in range — keeps them in heal range.
...
// Returns true if the unit's skill kit is dominated by heal/buff/shield skills.
...
// Returns the first ally (or self) below 30% HP.
...
// ── GRID ACCESS HELPERS ───────────────────────────────────────────
// Pathfinding needs raw grid + occupancy arrays, but grid-engine only exposes
// higher-level queries. We build minimal snapshots on demand.
...
// Runtime compatibility install — keep window.CJS.AIController identical to the
// legacy IIFE. The cast bridges the historical CJSAIController interface (the
// export also surfaces test-only _buildContext/_decodeRule/_archetypeDefault).
window.CJS = window.CJS || ({} as CJSNamespace);
window.CJS.AIController = AIController as unknown as CJSAIController;
