// ai-targeting.ts — Tier 3 TS port of js/ai/ai-targeting.js (engine cluster:
// ai). Pure functions for picking a target by named strategy (nearest/lowest_hp/
// squishiest/most_threatening/healer/pack_* …) and choosing the best AoE cell.
// Each strategy takes (attacker, candidates, grid) and returns { unit, score }
// or null.
//
// Reads: window.CJS GridEngine/AoE.
// Used by: ai-controller.
//
// Exports `AITargeting` and installs window.CJS.AITargeting (rides the
// CJSNamespace index signature). Bodies verbatim.

const GE = (): any => window.CJS.GridEngine;

// ── MAIN PICKER ────────────────────────────────────────────────────
// strategy: string (see header). attacker: the AI unit. allUnits: all live
// units (pulled from grid-engine if not provided).
// opts: { range?, requireLoS?, team?, skillId? }. Returns { unit, score } or null.
function pickTarget(strategy, attacker, allUnits, opts: any = {}) {
...
// Returns null when no cell hits at least one enemy.
function bestAoECell(attacker, aoeShape, aoeSize, range, opts: any = {}) {
...
// ── PUBLIC API ─────────────────────────────────────────────────────
export const AITargeting = Object.freeze({
  pickTarget,
  bestAoECell
});

// Runtime compatibility install — keep window.CJS.AITargeting identical to the
// legacy IIFE so every existing consumer (and the vanilla engine) is unchanged.
window.CJS = window.CJS || ({} as CJSNamespace);
window.CJS.AITargeting = AITargeting;
